...
def get_team_form(matches_df, team, n=5):
    """Get rolling last-N form for a team (home or away)."""
    team_matches = matches_df[(matches_df["home_team"] == team) | (matches_df["away_team"] == team)].tail(n)
    stats = []
    ...
    return {
        "avg_xg_scored": s["xg_scored"].mean(),
        "avg_xg_conceded": s["xg_conceded"].mean(),
        "avg_goals_scored": s["goals_scored"].mean(),
        "avg_goals_conceded": s["goals_conceded"].mean(),
        "avg_ppda":           s["ppda"].mean(),
        "avg_poss":           s["poss"].mean(),
        "avg_shots_total":    s["shots_total"].mean(),
        "avg_shots_ot":       s["shots_ot"].mean(),
        "avg_shot_acc":       s["shot_acc"].mean(),
        "avg_shot_distance":  s["shot_distance"].mean(),
        "avg_shots_against":  s["shots_against"].mean(),
        "avg_shots_against_ot": s["shots_against_ot"].mean(),
        "avg_pass_acc":       s["pass_acc"].mean(),
        "avg_off_duel_pct":   s["off_duel_pct"].mean(),
        "avg_def_duel_pct":   s["def_duel_pct"].mean(),
        "avg_touches_pa":     s["touches_pa"].mean(),
        "avg_corners": s["corners"].mean(),
        "avg_yellows": s["yellows"].mean(),
        "avg_reds":    s["reds"].mean(),
        "avg_fouls":   s["fouls"].mean(),
        "wins": int(wins),
        "draws": int(draws),
        "losses": int(losses),
        "n": len(s),
...
def predict(matches_df, home_team, away_team, n_form=5, max_goals=7):
    ...
    calibrated with xG rolling averages and PPDA pressing adjustment.
    """
    ...
    # League averages
    league_avg_xg = matches_df[["h_xg", "a_xg"]].values.mean()
    home_avg_xg = matches_df["h_xg"].mean()
    away_avg_xg = matches_df["a_xg"].mean()

    # Attack and defense indices (relative to league average)
    home_attack = home_form["avg_xg_scored"] / home_avg_xg
    home_defense = home_form["avg_xg_conceded"] / away_avg_xg
    away_attack = away_form["avg_xg_scored"] / away_avg_xg
    away_defense = away_form["avg_xg_conceded"] / home_avg_xg

    # Expected goals (lambda) — Dixon-Coles style
    # Home advantage factor: historically home teams score ~10-15% more
    home_advantage = 1.10

    lambda_home = home_attack * away_defense * home_avg_xg * home_advantage
    lambda_away = away_attack * home_defense * away_avg_xg

    # PPDA pressing adjustment: lower PPDA = better pressing = slight defensive boost
    # Normalize: league avg PPDA ~10, each point below avg gives 2% boost to defense
    ...
    # Apply PPDA: good pressing home team reduces lambda_away
    ...
    # Build score probability matrix using Poisson
    scores = {}
    prob_home_win = 0.0
    prob_draw = 0.0
    prob_away_win = 0.0
    ...
    # Normalize (residual probability from truncation)
    total = prob_home_win + prob_draw + prob_away_win
    prob_home_win /= total
    prob_draw /= total
    prob_away_win /= total

    # Normalize score probs too
